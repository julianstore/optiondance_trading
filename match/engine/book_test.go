...
const (
	RedisEngineCacheAddress  = "127.0.0.1:6379"
	RedisEngineCacheDatabase = 5
)
...
func TestBook(t *testing.T) {
	...
	cancelled := make([]*core.EngineOrder, 0)
	book := NewBook(ctx, "market", nil,
	...
	//book.AttachOrderEvent(ctx, bo1_1, &core.Action{
	//	OrderId:   bo1_1.Id,
	//	Action:    OrderActionCreate,
	//})

	id, _ = uuid.NewV4()
	bo1_2 := &core.EngineOrder{
		Id:              id.String(),
		Side:            PageSideBid,
		Type:            core.OrderTypeLimit,
		Price:           number.NewInteger(10000, 2),
	...
	//book.AttachOrderEvent(ctx, bo1_2, &core.Action{
	//	OrderId:   bo1_2.Id,
	//	Action:    OrderActionCreate,
	//})

	id, _ = uuid.NewV4()
	bo1_3 := &core.EngineOrder{
		Id:              id.String(),
		Side:            PageSideBid,
		Type:            core.OrderTypeLimit,
		Price:           number.NewInteger(10000, 2),
	...
	//book.AttachOrderEvent(ctx, bo1_3, &core.Action{
	//	OrderId:   bo1_3.Id,
	//	Action:    OrderActionCreate,
	//})
	...
	//book.AttachOrderEvent(ctx, bo2_1, &core.Action{
	//	OrderId:   bo2_1.Id,
	//	Action:    OrderActionCreate,
	//})

	id, _ = uuid.NewV4()
	ao1_1 := &core.EngineOrder{
		Id:              id.String(),
		Side:            PageSideAsk,
		Type:            core.OrderTypeLimit,
		Price:           number.NewInteger(10000, 2),
	...
	for i := 0; i < 2; i++ {
		//book.AttachOrderEvent(ctx, ao1_1, &core.Action{
		//	OrderId:   ao1_1.Id,
		//	Action:    OrderActionCreate,
		//})
		time.Sleep(100 * time.Millisecond)
	...
	//book.AttachOrderEvent(ctx, ao1_2, &core.Action{
	//	OrderId:   ao1_2.Id,
	//	Action:    OrderActionCreate,
	//})
	time.Sleep(100 * time.Millisecond)
	...
	//book.AttachOrderEvent(ctx, ao2_1, &core.Action{
	//	OrderId:   ao2_1.Id,
	//	Action:    OrderActionCreate,
	//})

	id, _ = uuid.NewV4()
	bo2_2 := &core.EngineOrder{
		Id:              id.String(),
		Side:            PageSideBid,
	...
	//book.AttachOrderEvent(ctx, bo2_2, &core.Action{
	//	OrderId:   bo2_2.Id,
	//	Action:    OrderActionCreate,
	//})
	time.Sleep(100 * time.Millisecond)
...
func testSetupRedis(ctx context.Context) context.Context {
	redisClient := redis.NewClient(&redis.Options{
		Addr:         RedisEngineCacheAddress,
		DB:           RedisEngineCacheDatabase,
...
func TestCancelOrderGoroutines(t *testing.T) {
	ctx := context.Background()
	matched := make([]*DummyTrade, 0)
	cancelled := make([]*core.EngineOrder, 0)
	...
	for i := 0; i < 10; i++ {
		marketName := "market" + strconv.Itoa(i)
		book := NewBook(ctx, marketName, nil,
			func(taker, maker *core.EngineOrder, amount number.Integer) string {
				matched = append(matched, &DummyTrade{
					Amount:           amount,
					TakerId:          taker.Id,
				...
				//t.Logf("market %s, order cancel start %s", marketName, order.Id)
				time.Sleep(time.Millisecond * time.Duration(rand.Int31n(1000)))
				t.Logf("market %s, order cancelled %s", marketName, order.Id)
				cancelled = append(cancelled, order)
			})
	...
	var (
		i         = 0
		orderList = make([]*core.EngineOrder, 0)
	)
	...
	for i < 100 {
		...
		for n := 0; n < 10; n++ {
			//bookList[n].AttachOrderEvent(ctx, order, &core.Action{
			//	OrderId:   order.Id,
			//	Action:    OrderActionCreate,
			//})
		}
	...
	for j := 0; j < 2; j++ {
		...
		//order := orderList[j]
		for m := 0; m < 5; m++ {
			//bookList[m].AttachOrderEvent(ctx, order, &core.Action{
			//	OrderId:   order.Id,
			//	Action:    OrderActionCancel,
			//})
			time.Sleep(100 * time.Millisecond)
